...
- **Certora folder**:
  - Certora folder is made up of 5 folders:
    - `config`: configuration files for the tool. There is one per contract. More can be added if needed.
    - `harness`: contracts that inherit base contracts to add extra functionality and other additional contracts.
    - `mock`: mocks needed for verification.
